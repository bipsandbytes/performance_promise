module ValidateFullTableScans
  def validate_full_table_scans(db_queries, render_time, promised)
    ...
    @full_table_scans & @promised_full_table_scans != @full_table_scans
  end

  def report_failed_full_table_scans(db_queries, render_time, promised)
    return "Promised table scans on #{@promised_full_table_scans}, made: #{@full_table_scans}", []
  end
end
